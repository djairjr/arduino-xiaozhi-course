...
void setup_network_setting(lv_screens_info *ui) {
    ...
    lv_obj_set_size(ui->menu_network_page_password_dialog, lv_pct(85), LV_SIZE_CONTENT);
    lv_obj_set_style_bg_color(ui->menu_network_page_password_dialog, lv_color_hex(0x85adad), 0);
    lv_obj_set_style_border_width(ui->menu_network_page_password_dialog, 0, 0);
    ...
    lv_obj_t *button_cont = lv_obj_create(ui->menu_network_page_password_dialog);
    lv_obj_remove_style_all(button_cont);
    lv_obj_set_size(button_cont, lv_pct(75), 30);
    ...
    lv_obj_t *confirm_button = lv_button_create(button_cont);
    lv_obj_t *cancel_button = lv_button_create(button_cont);
    lv_obj_set_size(confirm_button, 60, 30);
    lv_obj_set_size(cancel_button, 60, 30);
    lv_obj_set_style_bg_color(cancel_button, lv_color_hex(0xAA0000), 0);
    lv_obj_t *confirm_button_label = lv_label_create(confirm_button);
    lv_obj_t *cancel_button_label = lv_label_create(cancel_button);
    lv_label_set_text(confirm_button_label, "确定");
    lv_label_set_text(cancel_button_label, "取消");
    lv_obj_center(confirm_button_label);
    lv_obj_center(cancel_button_label);
    lv_obj_add_event_cb(confirm_button, hide_mast_event_callback, LV_EVENT_CLICKED, NULL);
    lv_obj_add_event_cb(cancel_button, hide_mast_event_callback, LV_EVENT_CLICKED, NULL);
