...
void setup_screen_settings(lv_screens_info *ui) {
    ...
    lv_obj_set_flex_align(ui->menu_network_page, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_all(ui->menu_network_page, 5, 0);
    ui->menu_network_page_cont = lv_menu_cont_create(ui->menu_network_page);

    ui->menu_network_page_tips_label = lv_label_create(ui->menu_network_page_cont);
    ui->menu_network_page_refresh_image = lv_image_create(ui->menu_network_page_cont);
    lv_image_set_src(ui->menu_network_page_refresh_image, LV_CUSTOM_SYMBOL_RESET);
    lv_label_set_text(ui->menu_network_page_tips_label, "连接附近的WLAN");

    ui->menu_network_page_wifi_list = lv_list_create(ui->menu_network_page);
    lv_obj_set_style_border_width(ui->menu_network_page_wifi_list, 1, 0);
    lv_obj_set_width(ui->menu_network_page_wifi_list, LV_HOR_RES - 10);
    lv_obj_set_flex_grow(ui->menu_network_page_wifi_list, 1);
